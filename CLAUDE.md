...
- `docs/sidebar-spec.md` : le mode sidebar (menu View, `cmd+option+s`, `[layout]` dans la conf), en v3 une vue AppKit native (`src/window/sidebar_view.rs`, SF Pro, tuiles arrondies, `NSScrollView`) posée à gauche de la vue Metal dans un conteneur : `KovaView::apply_layout` répartit la largeur, la vue Metal ne réserve plus rien pour la sidebar. Le modèle (`src/window/sidebar_model.rs`) est reconstruit à chaque tick et la vue ne se redessine que s'il a changé. Tuiles façon KovaLink avec actions au survol et au clic droit, la pilule Next (le bouton de Cmd+J) et la question d'un prompt de permission lue à l'écran (`src/prompt_preview.rs`, grammaire du daemon KovaLink, fixtures dans `tests/fixtures/prompt/`). `sidebar_width` est en points (200..520). Les changements de layout faits à chaud vont dans `~/.config/kova/prefs.json`, qui prend le pas sur `[layout]` de `config.toml` au chargement : Kova ne réécrit jamais le TOML.
